...
function wp_pagenavi( $args = array() ) {
	...
	if ( !empty( $options['pages_text'] ) ) {
		...
		$data['pages_text'] = [
			'paged' => number_format_i18n( $paged ),
			'total' => number_format_i18n( $total_pages )
		];
	}
	...
	if ( $paged > 1 && !empty( $options['prev_text'] ) ) {
		...
		$data['previous'] = $instance->get_url( $paged - 1 );
	}
	...
	foreach ( $larger_pages_array as $larger_page ) {
		if ( $larger_page < ($start_page - $half_page_start) && $larger_page_start < $larger_page_to_show ) {
			...
			// $data['smaller_pages'][] = $instance->get_url( $larger_page );
			$larger_page_start++;
		}
	}
	...
	$timeline = 'smaller';
	$data['page_numbers'] = [];
	$data['page_numbers']['smaller'] = [];
	$data['page_numbers']['larger'] = [];
	foreach ( range( $start_page, $end_page ) as $i ) {
		if ( $i == $paged && !empty( $options['current_text'] ) ) {
			...
			$data['page_numbers']['current'] = number_format_i18n( $i );
		} else {
			...
			$num_key = 'p_' . $i;
			$data['page_numbers'][ $timeline ][ $num_key ] = $instance->get_url( $i );
		}
	}
	...
	foreach ( $larger_pages_array as $larger_page ) {
		if ( $larger_page > ($end_page + $half_page_end) && $larger_page_end < $larger_page_to_show ) {
			...
			// $data['larger_pages'][] = $instance->get_url( $larger_page );
			$larger_page_end++;
		}
	}
	...
	if ( $paged < $total_pages && !empty( $options['next_text'] ) ) {
		...
		$data['next'] = $instance->get_url( $paged + 1 );
	}
	...
	if ( $end_page < $total_pages ) {
		...
		$data['last'] = $instance->get_url( $total_pages );
	}
