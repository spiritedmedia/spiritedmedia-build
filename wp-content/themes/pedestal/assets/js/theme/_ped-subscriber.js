/* global PedVars localStorageCookie PedUtils */
/* exported PedSubscriberClass, PedSubscriber */

/**
 * Subscriber functionality for the frontend
 */
class PedSubscriberClass {
  ...
  maybeRefresh() {
    ...
    var mcEid = PedUtils.getURLParams('mc_eid');
    var subscriberData = localStorageCookie(this.storageKey);
    if (subscriberData && 'data' in subscriberData) {
      var oldId = false;
...
const PedSubscriber = new PedSubscriberClass();
